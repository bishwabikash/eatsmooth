...
if(isset($_POST['signup'])){
    ...
    $addr=filter_var($_POST['log_addr'],FILTER_SANITIZE_STRING);
    $phn=$_POST['log_phn'];
    $pin=filter_var($_POST['log_pin'],FILTER_SANITIZE_STRING);
    $email=filter_var($_POST['log_email'],FILTER_SANITIZE_EMAIL);
    $stmt->prepare("INSERT into users(usr_phn, usr_nme, usr_passwd, usr_mail,usr_addr,usr_pin) VALUES (?,?,?,?,?,?)");
    $stmt->bind_param("ssssss",$phn,$uname,$pwd,$email,$addr,$pin);
    $stmt->execute();
    $stmt->close();
    ...
    $userqry=$db->query("SELECT usr_nme,usr_mail,usr_phn,usr_addr,usr_pin FROM users WHERE usr_nme LIKE '".$uname."' AND usr_passwd LIKE '".$passw."'");
    if ($userqry->num_rows==1){
        ...
        $_SESSION['uAddr']=$det['usr_addr'];
        $_SESSION['uPin']=intval($det['pin']);
        header("location:../index.php");
    }
}